    ...
    public class PatientRecordController : BaseController
    {
        ...
        public ActionResult Index(Guid? patientId)
        {
            ...
            using (var workScope = new UnitOfWork(new HospitalManagementDbContext()))
            {
                ...
                // Nếu là bác sĩ thì chỉ xem bệnh nhân của mình
                ...
                {
                    // Lọc bệnh án theo bệnh nhân cụ thể
                    var patient = workScope.Patients.FirstOrDefault(x => x.Id == patientId);
                    ViewBag.Patient = patient;

                    listData = workScope.PatientRecords
                        .Query(x => x.PatientId == patientId && !x.IsDelete)
                        .OrderByDescending(x => x.TestDate).ToList();

                    // Tạo dropdown danh sách bệnh nhân
                    ...
                    // Nếu là bác sĩ thì chỉ được xem bệnh án của chính mình
                    if (user.Role == RoleKey.Doctor)
                    {
                ...
                // Trường hợp không truyền id bệnh nhân, lấy tất cả bệnh án
                ViewBag.Patients = new SelectList(patients.Select(x => new
                {
                    id = x.Id,
                    FullName = x.PatientCode + " - " + x.FullName
                ...
                // Nếu là bác sĩ, chỉ xem hồ sơ của chính mình
                if (user.Role == RoleKey.Doctor)
                {
                    listData = listData.Where(x => x.DoctorId == user.DoctorId).ToList();
                }
        ...
        public ActionResult Create(Guid? patientId)
        {
            ...
            using (var workScope = new UnitOfWork(new HospitalManagementDbContext()))
            {
                ...
                // Lọc bệnh nhân theo bác sĩ nếu là bác sĩ
                if (user.Role == RoleKey.Doctor)
                {
                    var patientOfDoctors = workScope.PatientDoctors.Query(x => x.DoctorId == user.DoctorId) ?? new List<PatientDoctor>();
                    var patientOfDoctorIds = patientOfDoctors.Select(x => x.PatientId);
                ...
                // Gửi danh sách bệnh nhân lên View
                ...
                // Khởi tạo thông tin mặc định cho bệnh án mới
        ...
        public ActionResult Update(Guid id)
        {
            ...
            using (var workScope = new UnitOfWork(new HospitalManagementDbContext()))
            {
                ...
                var patientRecord = workScope.PatientRecords.FirstOrDefault(x => x.Id == id);
                ...
                return View("Create", patientRecord); // Dùng chung View Create
            }
        }

        // Xử lý lưu (tạo hoặc cập nhật) bệnh án
        ...
        public JsonResult CreateOrEdit(PatientRecord input, TimeSpan testTime, bool isEdit)
        {
            try
            {
                ...
                if (isEdit) // Trường hợp cập nhật
                {
                    using (var workScope = new UnitOfWork(new HospitalManagementDbContext()))
                    {
                        ...
                        if (elm != null)
                        {
                            ...
                            return Json(new { status = true, mess = "Cập nhập thành công ", data = new { input.RecordId } });
                        }
                ...
                using (var workScope = new UnitOfWork(new HospitalManagementDbContext()))
                {
                    ...
                    // Tạo chi tiết bản ghi chính (bệnh chính)
                ...
                return Json(new { status = true, mess = "Thêm thành công " + KeyElement, data = new { input.RecordId } });
            }
            catch (Exception ex)
            {
        ...
        // Xóa bệnh án (xóa mềm, kiểm tra có thuốc hay không)
        [HttpPost]
            ...
            try
            {
                using (var workScope = new UnitOfWork(new HospitalManagementDbContext()))
                {
                    ...
                    if (elm != null)
                    {
                        ...
                        // Kiểm tra nếu có thuốc kê → không được xóa
