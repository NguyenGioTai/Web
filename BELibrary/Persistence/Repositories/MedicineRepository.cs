using BELibrary.Core.Entity.Repositories; // Import namespace ch?a interface IMedicineRepository.
using BELibrary.DbContext; // Import namespace ch?a l?p HospitalManagementDbContext (DbContext c?a ?ng d?ng).
using BELibrary.Entity; // Import namespace ch?a l?p Medicine (Entity ð?i di?n cho thu?c).

namespace BELibrary.Persistence.Repositories // Namespace ð?nh ngh?a nõi ch?a các l?p repository ð? truy c?p d? li?u.
{
    /// <summary>
    /// L?p MedicineRepository ch?u trách nhi?m th?c hi?n các thao tác truy v?n d? li?u liên quan ð?n ð?i tý?ng Medicine.
    /// K? th?a t? l?p Repository<Medicine> ð? t?n d?ng các phýõng th?c truy v?n d? li?u cõ b?n (CRUD).
    /// Ð?ng th?i tri?n khai interface IMedicineRepository ð? ð?m b?o l?p này cung c?p các phýõng th?c ð?c thù (n?u có) cho Medicine.
    /// </summary>
    public class MedicineRepository : Repository<Medicine>, IMedicineRepository
    {
        /// <summary>
        /// Hàm kh?i t?o c?a l?p MedicineRepository.
        /// Nh?n m?t instance c?a HospitalManagementDbContext thông qua dependency injection.
        /// G?i constructor c?a l?p cha (Repository<Medicine>) và truy?n context này vào.
        /// </summary>
