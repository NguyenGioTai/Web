...
namespace BELibrary.Migrations
{
    ...
    using BELibrary.Core.Utils; // Chứa các hằng số và tiện ích như VariableExtensions, RoleKey
    using BELibrary.DbContext; // Chứa HospitalManagementDbContext
    using BELibrary.Entity; // Chứa các thực thể như Account, Faculty, Medicine
    using BELibrary.Utils; // Chứa CryptorEngine để mã hóa mật khẩu
    using System;
    using System.Collections.Generic;
    using System.Data.Entity.Migrations;

    // Lớp Configuration cấu hình migration cho HospitalManagementDbContext
    internal sealed class Configuration : DbMigrationsConfiguration<HospitalManagementDbContext>
    {
        // Constructor khởi tạo cấu hình migration
        public Configuration()
        {
            // Tắt tự động migration để yêu cầu chạy migration thủ công
            // Điều này giúp kiểm soát chặt chẽ các thay đổi cơ sở dữ liệu
            AutomaticMigrationsEnabled = false;
        }

        // Phương thức Seed thêm dữ liệu khởi tạo vào cơ sở dữ liệu sau khi migration
        protected override void Seed(HospitalManagementDbContext context)
        {
            // Đoạn code bị comment, có thể dùng để thêm vai trò (Role) vào bảng Roles
            // Role role = new Role
            // {
            //     Id = Guid.NewGuid(),
            ...
            // Tạo mật khẩu cho tài khoản admin bằng cách kết hợp mật khẩu mặc định và khóa mã hóa
            var passwordFactory = VariableExtensions.DefautlPassword + VariableExtensions.KeyCrypto;
            // Mã hóa mật khẩu sử dụng CryptorEngine với tùy chọn hashing
            var passwordCrypto = CryptorEngine.Encrypt(passwordFactory, true);

            // Thêm hoặc cập nhật tài khoản admin vào bảng Accounts
            context.Accounts.AddOrUpdate(c => c.UserName, new Account()
            {
                Id = Guid.NewGuid(), // Tạo ID mới
                FullName = "Quản Trị", // Họ tên
                Phone = "0973 642 032", // Số điện thoại
                UserName = "admin", // Tên đăng nhập
                Password = passwordCrypto, // Mật khẩu đã mã hóa
                Role = 1, // Vai trò Admin (theo RoleKey.Admin)
                Gender = true, // Giới tính: Nam
                LinkAvatar = "" // Không có ảnh đại diện
            });

            #region Faculty

            // Tạo danh sách các khoa/phòng ban (Faculty) để khởi tạo
            var faculties = new List<Faculty>
            {
                ...
                new Faculty { Id = Guid.NewGuid(), Name = "Khoa Y học cổ truyền - Phục hồi chức năng" }, // Trùng lặp
                ...
                new Faculty { Id = Guid.NewGuid(), Name = "Khoa Khám và Điều trị theo yêu cầu" }, // Trùng lặp
                ...
                new Faculty { Id = Guid.NewGuid(), Name = "Phòng Chỉ đạo tuyến" }, // Trùng lặp
            ...
            // Đoạn code bị comment, có thể dùng để thêm các khoa vào bảng Faculties
            ...
            // Tạo danh sách các loại thuốc (Medicine) để khởi tạo
            var medicines = new List<Medicine>
            {
                ...
                new Medicine { Id = Guid.NewGuid(), Name = "Abobotulinum Toxin A", Description = "" }, // Trùng lặp
                ...
                new Medicine { Id = Guid.NewGuid(), Name = "Alendronate", Description = "" }, // Trùng lặp
            ...
            // Đoạn code bị comment, có thể dùng để thêm các thuốc vào bảng Medicines
